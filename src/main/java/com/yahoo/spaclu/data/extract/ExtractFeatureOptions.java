...
public class ExtractFeatureOptions {
	final Logger sLogger = LoggerFactory.getLogger(ExtractFeatureOptions.class);
	...
	public ExtractFeatureOptions(String args[]) {
		...
		try {
			...
			if (line.hasOption(Settings.HELP_OPTION)) {
				formatter.printHelp(ExtractFeatureSpark.class.getName(),
						options);
				System.exit(0);
			}
			 ...
			 * .getOptionValue(Settings.MAX_CUTOFF_OPTION)); } if
			 * (line.hasOption(Settings.MIN_CUTOFF_OPTION)) {
			 * minimumCutoffThreshold = Integer.parseInt(line
			 * .getOptionValue(Settings.MIN_CUTOFF_OPTION)); }
			 */
		} catch (ParseException pe) {
			sLogger.error(pe.getMessage());
			formatter.printHelp(ExtractFeatureSpark.class.getName(), options);
			System.exit(0);
	...
	public String toString() {
		return ExtractFeatureSpark.class.getSimpleName();
	}
}